$(function () {
    ...
    // 텝메뉴
    var cateTab = new TabMenu('.product_view .tab_menu');
...
NavUI.prototype._addOff = function(){
    this.$selectItem.not().siblings().addClass('off');
    this.$selectItem.removeClass('off');
}
NavUI.prototype._removeOff = function(){
    this.$gnbItem.removeClass('off');
}
NavUI.prototype._menuOpen = function () {
    this.$subMenuClose.stop().slideUp(100);
    this.$subMenuOpen.stop().slideDown(200);
};
...
ScrollAction.prototype._init = function (parallaxNav, parallaxCon) {
    ...
    this.atOffsetPosition = this.$parallaxNav.offset().top - 105;
