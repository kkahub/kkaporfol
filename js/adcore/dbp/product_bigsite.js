$(function() {
	$('#cdr').circleProgress({
		...
		emptyFill : 'rgba(230, 230, 230, .3)',
		startAngle : -Math.PI / 2
	});
	$('#subway').circleProgress({
		value : 0.35,
		size : 200,
		thickness : 40,
		fill : {
	...
	/* 서브 헤더 시작 */
		function subHeader(){
			if ($(window).scrollTop() > 235) {
				$(".bigsite_snb").css("position","fixed").css("top", "80px").css("left",0).css("z-index","1").css("width","100%");
				$(".page-wrap.media").css("margin-top","155px");
			} else {
				$(".bigsite_snb").css("position","relative").css("top","0").css("left",0);
	...
	$(window).load(function() {
		$('.milestone-count').text($('.milestone-count').text() + "%");
	});
